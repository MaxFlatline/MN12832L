...
//    for (size_t i = 0; i < 32; i++){        // Cycle for original frame's rows
        for (size_t j = 0; j < 15; j = j + 3){    // Cycle for original frame's columns
                ...
                VFD_RowData_ABC[1][0] |= ((image[0 + 16] & 0x02) != 0x02) << 6; // a11
                VFD_RowData_ABC[1][1]  = ((image[0 + 16] & 0x01) != 0x01) << 0; // b11
                
                //first byte of third row of bitmap
                VFD_RowData_ABC[0][1] |= ((image[0 + 32] & 0x80) != 0x80) << 4; // a20
                VFD_RowData_ABC[0][1] |= ((image[0 + 32] & 0x40) != 0x40) << 6; // b20
                VFD_RowData_ABC[0][2]  = ((image[0 + 32] & 0x20) != 0x20) << 0; // c20
                
                VFD_RowData_DEF[0][2]  = ((image[0 + 32] & 0x10) != 0x10) << 1; // d20
                VFD_RowData_DEF[0][1] |= ((image[0 + 32] & 0x08) != 0x08) << 7; // e20
                VFD_RowData_DEF[0][1] |= ((image[0 + 32] & 0x04) != 0x04) << 5; // f20
                
                VFD_RowData_ABC[1][1] |= ((image[0 + 16] & 0x02) != 0x02) << 4; // a21
                VFD_RowData_ABC[1][1] |= ((image[0 + 16] & 0x01) != 0x01) << 6; // b21
                //first byte of fourth row of bitmap
                VFD_RowData_ABC[0][2] |= ((image[0 + 48] & 0x80) != 0x80) << 2; // a30
                VFD_RowData_ABC[0][2] |= ((image[0 + 48] & 0x40) != 0x40) << 4; // b30
                VFD_RowData_ABC[0][2] |= ((image[0 + 48] & 0x20) != 0x20) << 6; // c30
                
                VFD_RowData_DEF[0][2] |= ((image[0 + 48] & 0x10) != 0x10) << 7; // d30
                VFD_RowData_DEF[0][2] |= ((image[0 + 48] & 0x08) != 0x08) << 5; // e30
                VFD_RowData_DEF[0][2] |= ((image[0 + 48] & 0x04) != 0x04) << 3; // f30
                
                VFD_RowData_ABC[1][2]  = ((image[0 + 48] & 0x02) != 0x02) << 2; // a31
                VFD_RowData_ABC[1][2] |= ((image[0 + 48] & 0x01) != 0x01) << 4; // b31
                ...
                VFD_RowData_ABC[1][1] |= ((image[1 + 16] & 0x80) != 0x80) << 2; // c11
                VFD_RowData_DEF[1][1] |= ((image[1 + 16] & 0x40) != 0x40) << 3; // d11
                VFD_RowData_DEF[1][1] |= ((image[1 + 16] & 0x20) != 0x20) << 1; // e11
                VFD_RowData_DEF[1][0] |= ((image[1 + 16] & 0x10) != 0x10) << 7; // f11
                
                VFD_RowData_ABC[2][0] |= ((image[1 + 16] & 0x08) != 0x08) << 6; // a12
                VFD_RowData_ABC[2][1]  = ((image[1 + 16] & 0x04) != 0x04) << 0; // b12
                VFD_RowData_ABC[2][1] |= ((image[1 + 16] & 0x02) != 0x02) << 2; // c12
                VFD_RowData_DEF[2][1]  = ((image[1 + 16] & 0x01) != 0x01) << 3; // d12
                //third byte of second row
                VFD_RowData_ABC[1][2] |= ((image[1 + 32] & 0x80) != 0x80) << 0; // c21
                VFD_RowData_DEF[1][2]  = ((image[1 + 32] & 0x40) != 0x40) << 1; // d21
                VFD_RowData_DEF[1][1] |= ((image[1 + 32] & 0x20) != 0x20) << 7; // e21
                VFD_RowData_DEF[1][1] |= ((image[1 + 32] & 0x10) != 0x10) << 5; // f21
                
                VFD_RowData_ABC[2][1] |= ((image[1 + 32] & 0x08) != 0x08) << 4; // a22
                VFD_RowData_ABC[2][1] |= ((image[1 + 32] & 0x04) != 0x04) << 6; // b22
                VFD_RowData_ABC[2][2] |= ((image[1 + 32] & 0x02) != 0x02) << 0; // c22
                VFD_RowData_DEF[2][2]  = ((image[1 + 32] & 0x01) != 0x01) << 1; // d22
                //forth byte of second row
                VFD_RowData_ABC[1][2] |= ((image[1 + 48] & 0x80) != 0x80) << 6; // c31
                VFD_RowData_DEF[1][2] |= ((image[1 + 48] & 0x40) != 0x40) << 7; // d31
                VFD_RowData_DEF[1][2] |= ((image[1 + 48] & 0x20) != 0x20) << 5; // e31
                VFD_RowData_DEF[1][2] |= ((image[1 + 48] & 0x10) != 0x10) << 3; // f31
                
                VFD_RowData_ABC[2][2] |= ((image[1 + 48] & 0x08) != 0x08) << 2; // a32
                VFD_RowData_ABC[2][2] |= ((image[1 + 48] & 0x04) != 0x04) << 4; // b32
                VFD_RowData_ABC[2][2] |= ((image[1 + 48] & 0x02) != 0x02) << 6; // c32
                VFD_RowData_DEF[2][2] |= ((image[1 + 48] & 0x01) != 0x01) << 7; // d32
                ...
                //third byte of second row
                VFD_RowData_DEF[2][1] |= ((image[2 + 16] & 0x80) != 0x80) << 1; // e12
                VFD_RowData_DEF[2][0] |= ((image[2 + 16] & 0x40) != 0x40) << 7; // f12
                
                VFD_RowData_ABC[3][0] |= ((image[2 + 16] & 0x20) != 0x20) << 6; // a13
                VFD_RowData_ABC[3][1]  = ((image[2 + 16] & 0x10) != 0x10) << 0; // b13
                VFD_RowData_ABC[3][1] |= ((image[2 + 16] & 0x08) != 0x08) << 2; // c13
                VFD_RowData_DEF[3][1]  = ((image[2 + 16] & 0x04) != 0x04) << 3; // d13
                VFD_RowData_DEF[3][1] |= ((image[2 + 16] & 0x02) != 0x02) << 1; // e13
                VFD_RowData_DEF[3][0] |= ((image[2 + 16] & 0x01) != 0x01) << 7; // f13
                //third byte of third row
                VFD_RowData_DEF[2][1] |= ((image[2 + 32] & 0x80) != 0x80) << 7; // e22
                VFD_RowData_DEF[2][1] |= ((image[2 + 32] & 0x40) != 0x40) << 5; // f22
                
                VFD_RowData_ABC[3][1] |= ((image[2 + 32] & 0x20) != 0x20) << 4; // a23
                VFD_RowData_ABC[3][1] |= ((image[2 + 32] & 0x10) != 0x10) << 6; // b23
                VFD_RowData_ABC[3][2]  = ((image[2 + 32] & 0x08) != 0x08) << 0; // c23
                VFD_RowData_DEF[3][2]  = ((image[2 + 32] & 0x04) != 0x04) << 1; // d23
                VFD_RowData_DEF[3][1] |= ((image[2 + 32] & 0x02) != 0x02) << 7; // e23
                VFD_RowData_DEF[3][1] |= ((image[2 + 32] & 0x01) != 0x01) << 5; // f23
                //third byte of forth row
                VFD_RowData_DEF[2][2] |= ((image[2 + 48] & 0x80) != 0x80) << 5; // e32
                VFD_RowData_DEF[2][2] |= ((image[2 + 48] & 0x40) != 0x40) << 3; // f32
                
                VFD_RowData_ABC[3][2] |= ((image[2 + 48] & 0x20) != 0x20) << 2; // a33
                VFD_RowData_ABC[3][2] |= ((image[2 + 48] & 0x10) != 0x10) << 4; // b33
                VFD_RowData_ABC[3][2] |= ((image[2 + 48] & 0x08) != 0x08) << 6; // c33
                VFD_RowData_DEF[3][2] |= ((image[2 + 48] & 0x04) != 0x04) << 7; // d33
                VFD_RowData_DEF[3][2] |= ((image[2 + 48] & 0x02) != 0x02) << 5; // e33
                VFD_RowData_DEF[3][2] |= ((image[2 + 48] & 0x01) != 0x01) << 3; // f33
            }
